...
def moving_status_callback(msg):
    ...
    if status == 1:  # 상태 리스트가 비어있지 않을때만 처리
        if gui_process1 is None:
            ...
            if gui_process2:  # gui_process2가 실행 중이라면 종료
                ...
                gui_process2 = None
    elif status == 3:  # /move_base/result == 3 이면 목적지 도착
...
def main():
    ...
    rospy.Subscriber('/move_base/result', MoveBaseActionResult, moving_status_callback)
